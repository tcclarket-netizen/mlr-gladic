...
function buildSectionBodies(input: BuildLegalReportInput): Record<string, string> {
  ...
  const section4 = [
    ...
    `GLADIC AI™ Average Score (3-bureau): ${avgScoreLabel}`,
    snap ? `Available-bureau average (current): ${metrics.average_score ?? "N/A"}` : "",
    snap ? formatUtilizationLine(snap) : "",
    `GLADIC AI™ Average Usage (3-bureau): ${avgUtilLabel}`,
    `GLADIC AIR (Annual Inquiry Rate proxy): ${metrics.inquiry_rate ?? "N/A"}`,
    "Executive View:",
  ...
  return {
    ...
    "21": [
      ...
      "Template 1 — CRA Dispute + Method-of-Verification Request (15 U.S.C. § 1681i(a)(6)): Use standard GLADIC dispute pack.",
      "",
      "Exhibit E: Agency Filing Pack (State AG + CFPB + FTC) — Templates",
      "Use state safe harbor filing templates included in GLADIC agency filing center.",
      "",
